...
It's always a good idea to do some recon work first. What do the data look like? How many observations and variables do we have? What are the variable names?
 ...
 It looks like this data is showing genres of movies from 1939 until the 2002, It shows the title of the 
movie the year of it, the length of the movie, the budget to make the movie, and the rating of the movie to 
see if people liked it.
...
median(newdata)
```
 The median budget for the movie is at 3,000,000
...
- What is the median budget for a movie? The median budget for the movie is at 3,000,000
...
movies_1995 <- movies %>%    
  ...
  drop_na(budget) %>%
  select(title, rating, budget, year)

movies_1995
```

There was 135 movies released in 1995.
...
movies_1995 <- movies %>%    
  filter(year >= 1995 & Action == 1) %>%
  drop_na(budget) %>%
  select(Comedy,title, rating, length, year)

movies_1995
```

There were no records found for movies with a rating above an 8 with 120 minutes.
...
The movies that are included in this data are Harry Potter and the Chamber of Secrets,Harry Potter and the Prisoner of Azkaban and Harry Potter and the Sorcerer's Stone. The harry porter movie with the highest rating is Harry Potter and the Prisoner of Azkaban. The movie that has the most votes on  imdb.com is Harry Potter and the Sorcerer's Stone.
...
movies_1995 <- movies %>%    
  filter(year >= 1995 & Action == 1) %>%
  drop_na(budget) %>%
  select(title,Comedy,Animation, votes, budget, year)

movies_1995
```

The highest budget for a movie that is an Animation or Comedy with at least 100 votes is Spider-Man 2 2004.
...
My favorite movie is not in the data set.
